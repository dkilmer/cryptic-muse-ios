...
class SQLiteDB {
	...
	private static var __once: () = {
			Static.instance = SQLiteDB.init(gid:"")
		}()
	...
	private func prepare(_ sql:String, params:[AnyObject]?)->OpaquePointer? {
...
//				println("Binding: \(params![ndx-1]) at Index: \(ndx)")
	...
	private func execute(_ stmt:OpaquePointer, sql:String)->CInt {
		// Step
	...
	private func query(_ stmt:OpaquePointer, sql:String)->[[String:AnyObject]] {
		var rows = [[String:AnyObject]]()
...
//						println("Column type:\(type) with value:\(val)")
	...
	// Get column type
	private func getColumnType(_ index:CInt, stmt:OpaquePointer)->CInt {
		var type:CInt = 0
...
//		println("SQLiteDB - Got column type: \(buf)")
		if buf != nil {
      var tmp = String.init(validatingUTF8: buf!)!.uppercased()
			// Remove brackets
...
//			println("SQLiteDB - Cleaned up column type: \(tmp)")
	...
	// Get column value
	private func getColumnValue(_ index:CInt, type:CInt, stmt:OpaquePointer)->AnyObject? {
		// Integer
